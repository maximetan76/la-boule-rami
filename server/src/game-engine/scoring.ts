 ...
 *
 * Réf. docs/REGLES.md § « Fin d'un coup et scoring » : celui qui a annoncé
 * « Je joue » — et donc clos la phase d'annonces — sans avoir réussi sa
 * première pose à aucun moment du coup, quand un autre joueur termine. Une
...
export const joueurChocolat = (coup: Coup, gagnantId: JoueurId): JoueurId | null => {
  const annonceur = coup.ordreJoueurs.find((joueurId) => coup.annonces[joueurId] === 'je-joue');
